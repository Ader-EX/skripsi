...
const TimeTableView = ({ schedules, rooms, timeSlots, filters }) => {
  ...
  const handleConflictClick = (conflictDetails) => {
    setSelectedConflict(conflictDetails);
  };
    ...
    return (
      ...
      >
        <DialogContent>
          ...
          <div>
            {selectedConflict.map((conflict, index) => (
              <div key={index}>
                <p>{conflict.reason}</p>
                <p>Severity: {conflict.severity}</p>
                <p>
                  Conflicting Timetable ID: {conflict.conflicting_timetable_id}
                </p>
                <p>Room: {conflict.room_id}</p>
                <p>Timeslot: {conflict.timeslot_id}</p>
              </div>
            ))}
          </div>
          <DialogFooter>
  ...
  const renderScheduleCard = (scheduleList, timeSlot, roomId) => {
            ...
            <TooltipTrigger asChild>
              <div
                className={`h-full w-full p-2 ${
                  schedule.is_conflicted ? "bg-red-100" : "bg-green-100"
                }`}
                onClick={() =>
                  schedule.is_conflicted &&
                  handleConflictClick(schedule.conflict_details)
                }
              >
    ...
    return (
      <div className="h-full w-full grid grid-cols-2 gap-0.5 p-0.5 bg-red-100">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  <div className="truncate">{schedule.subject.code}</div>
                </div>
              </TooltipTrigger>
              <TooltipContent>
                <p>{schedule.subject.name}</p>
