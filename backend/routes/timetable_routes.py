...
def initialize_population(
    ...
    for _ in range(population_size):
        ...
        for pengajaran in pengajaran_list:
            ...
            available_timeslots = random.sample(timeslots, sks)
            timetable.append({
...
def calculate_fitness(timetable: List[Dict], pengajaran_cache: Dict, room_cache: Dict) -> int:
    ...
    for entry in timetable:
        ...
        room_id = entry["ruangan_id"]

        # Lecturer conflict
        lecturer_id = pengajaran_cache[pengajaran_id]["dosen_id"]
        ...
        # Room conflict
        if room_id not in room_timeslot_map:
            room_timeslot_map[room_id] = set()
        if any(ts_id in room_timeslot_map[room_id] for ts_id in timeslot_ids):
            total_penalty += 20
            entry["is_conflicted"] = True
        room_timeslot_map[room_id].update(timeslot_ids)
...
def evolve_population(population: List[List[Dict]], pengajaran_cache: Dict, room_cache: Dict, timeslots: List[TimeSlot]) -> List[List[Dict]]:
    ...
    for timetable in new_population:
        if random.random() < 0.1:
            ...
            gene["timeslot_ids"] = [random.choice(timeslots).id for _ in range(len(gene["timeslot_ids"]))]
