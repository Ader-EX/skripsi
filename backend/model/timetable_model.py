...
class TimeTable(Base):
    ...
    temporary_timetables: Mapped[list["TemporaryTimeTable"]] = relationship("TemporaryTimeTable", back_populates="timetable", cascade="all, delete-orphan")
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    opened_class_id: Mapped[int] = mapped_column(ForeignKey("opened_class.id"), nullable=False)  # Add this line
    ruangan_id: Mapped[int] = mapped_column(ForeignKey("ruangan.id"), nullable=False)
    timeslot_ids: Mapped[List[int]] = mapped_column(JSON, nullable=False)  # List of timeslot IDs
    is_conflicted: Mapped[bool] = mapped_column(Boolean, default=False)
